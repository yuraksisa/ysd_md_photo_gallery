...
# Using it:
# 
#  adapter = MediaIntegration::Adapters::PicasaAdapter.new('username','password', true)
...
module MediaIntegration
  ...
  module Adapters
    ...
    class PicasaAdapter
      ...
      #
      # Execute a block within a client connection
      #
      def with_client 
    ...
    private
      ...
      def entry_to_photo(entry, photo)
        ...
        entry.elements['media:group'].elements.each('media:thumbnail') do |thumbnail|
          pt = MediaIntegration::PhotoThumbnail.new(thumbnail.attributes['url'], thumbnail.attributes['width'], thumbnail.attributes['height'])
          photo.thumbnails << pt
        end    
